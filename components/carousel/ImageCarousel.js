...
const ImageCarousel = ({ images }) => {
  return (
    ...
    renderArrowPrev={(clickHandler, hasPrev) => {
      return (
        <div onClick={clickHandler} >
          <ArrowBackIosIcon className={classes.arrowColor} />
        </div>
      );
    }}
    renderArrowNext={(clickHandler, hasNext) => {
      return (
        <div onClick={clickHandler}>
          <ArrowForwardIosIcon className={classes.arrowColor} />
        </div>
      );
    }}
    >
      {images.map((image, index) => (
        <div className={classes.caroconatiner} >
          <div key={index} className={classes.sideImage}>
            <Image
              src={image.url}
              alt={`Image ${index}`}
              width={570}
